...
export const POST = async (req) => {
  try {
    ...
    const description = formData.get('description').trim(); // Trim episode description
    const coverDescription = formData.get('coverDescription')?.trim(); // Trim cover image description (optional)
    ...
    // Upload the cover image (if provided) with description in metadata
    if (coverBuffer) {
        ...
        cloudinary.v2.uploader.upload_stream(
          {
            ...
            context: coverDescription ? `description=${coverDescription}` : '', // Add cover description as metadata if provided
          },
          (error, result) => {
      ...
      cloudinary.v2.uploader.upload_stream(
        {
          ...
          context: `description=${description}`, // Add episode description as metadata
        },
        (error, result) => {
          if (error) return reject(error);
          resolve(result);
